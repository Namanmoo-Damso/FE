...
export default function InstitutionMonitoringPage() {
  ...
  const filteredUsers = getFilteredUsers(MOCK_USERS, filter, searchTerm).slice(
    0,
    16,
  );
      ...
      <div className="flex-1 flex p-8 gap-8 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col gap-6">
          <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
            <h4 className="text-slate-400 font-black text-xs uppercase tracking-widest mb-4">
              AI 실시간 분석 로그
            </h4>
            <div className="space-y-4 text-slate-800">
              ...
              <div className="flex gap-3">
                <div className="p-2 bg-emerald-500 text-white rounded-lg h-fit">
                  <ScanFace size={18} />
                </div>
                <div>
...
function CompactUserCard({
  ...
  return (
    <div
      onClick={onExpand}
      className={`group relative bg-white rounded-xl overflow-hidden border-2 cursor-pointer transition-all hover:shadow-lg active:scale-95 ${
        isEmergency
          ? 'border-[#C05656] ring-2 ring-red-500/10'
    ...
    >
      ...
      <div className="absolute inset-0 p-3 flex flex-col justify-between bg-gradient-to-t from-slate-900/40 via-transparent to-transparent">
        <div className="flex justify-between items-start">
          <span className="text-slate-800 bg-white/80 backdrop-blur-sm px-1.5 py-0.5 rounded text-[11px] font-black">
            {user.name}
          </span>
          <div
            className={`w-2 h-2 rounded-full ${isEmergency ? 'bg-red-500 animate-ping' : 'bg-emerald-500'}`}
          />
        </div>
        ...
        <div className="text-[9px] text-white font-bold opacity-80 tracking-widest">
          {user.id}
        </div>
      </div>
    </div>
