...
const INSTITUTION_ID = process.env.NEXT_PUBLIC_INSTITUTION_ID || "INST-004"
    ...
    return (
        <div className="flex flex-col h-screen bg-slate-50">
            ...
            <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-10 flex-shrink-0 z-10 shadow-sm">
                <div>
                    <h1 className="text-2xl font-black text-slate-800 tracking-tight">대상자 관리</h1>
                    <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-wider">기관번호: {INSTITUTION_ID}</p>
                </div>
                    ...
                    <div className="bg-white rounded-[32px] shadow-sm border border-slate-200 overflow-hidden ring-1 ring-slate-200/50">
                            ...
                            <table className="w-full text-left border-collapse min-w-[1100px]">
                                <thead className="bg-slate-50/80 border-b border-slate-200">
                                    <tr>
                                        ...
                                        <th className="px-8 py-5 text-xs font-black uppercase tracking-widest text-slate-400">담당자</th>
                                    ...
                                    {filteredUsers.map((user) => (
                                        <tr key={user.id} className="group hover:bg-slate-50 transition-colors">
                                                ...
                                                <div className="flex items-center gap-5">
                                                    ...
                                                    <div className="min-w-0">
                                                        <p className="font-black text-slate-800 text-base tracking-tight truncate">
                                                            {user.name} <span className="font-bold text-[10px] text-slate-400 bg-slate-50 px-2 py-0.5 border rounded-lg ml-1.5">{user.age}세/{user.gender}</span>
                                                        </p>
                                            ...
                                            <td className="px-8 py-6">
                                                ...
                                                <p className="text-xs text-slate-400 mt-3 font-bold truncate max-w-[220px]" title={user.mainCondition}>
                                                    질환: {user.mainCondition}
                                                </p>
                                            </td>
                                            <td className="px-8 py-6">
                                                <div className="flex items-center gap-2 mb-2.5">
                                                    <div className="p-1.5 bg-slate-100 rounded-lg text-[#4F5B75]"><Bot size={14}/></div>
                                            ...
                                            <td className="px-8 py-6">
                                                <div className="flex justify-center gap-3">
                                                    <Link href={`/institution/workspace/users/${user.id}`}>
                                                        <button className="p-3 text-slate-400 hover:text-[#4F5B75] hover:bg-white rounded-2xl shadow-sm border border-transparent hover:border-slate-200 transition-all" title="상세보기">
                                                            <ArrowRight size={20} />
                                                        </button>
                                                    </Link>
                                                    <button className="p-3 text-slate-400 hover:text-emerald-600 hover:bg-white rounded-2xl shadow-sm border border-transparent hover:border-emerald-100 transition-all" title="통화">
                                                        <Phone size={20} />
                                                    </button>
                        ...
                        {filteredUsers.length === 0 && (
                            <div className="p-32 text-center bg-slate-50/30 uppercase tracking-[0.2em]">
                                ...
                                <p className="text-xs font-black text-slate-400">데이터가 존재하지 않습니다.</p>
                            </div>
                        )}
                    </div>
                )}
...
function FilterButton({ label, count, active, onClick, isCritical }: any) {
    return (
        <button
            onClick={onClick}
            className={`px-6 py-3 rounded-xl text-xs font-black uppercase tracking-wider transition-all flex items-center gap-3 ${active
                ? (isCritical ? "bg-[#C05656] text-white shadow-lg shadow-red-100" : "bg-[#4F5B75] text-white shadow-lg shadow-slate-200")
                : "text-slate-400 hover:text-slate-900"
                }`}
        >
            {label}
            <span className={`text-[10px] px-2 py-0.5 rounded-full font-black ${active ? "bg-black/20 text-white" : "bg-slate-100 text-slate-500"}`}>
                {count}
            </span>
        </button>
    )
