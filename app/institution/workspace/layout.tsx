...
const SIDEBAR_MENU = [
    { label: "대시보드", icon: LayoutDashboard, href: "/institution/workspace/dashboard" },
    { label: "실시간 모니터링", icon: Video, href: "/institution/workspace/monitoring" },
    { label: "대상자 관리", icon: Users, href: "/institution/workspace/users" },
    { label: "담당자 관리", icon: UserCog, href: "/institution/workspace/staff" },
    { label: "AI 설정 및 스크립트", icon: BrainCircuit, href: "/institution/workspace/ai-config" },
    { label: "알림 · 대응 기록", icon: Bell, href: "/institution/workspace/alerts" },
    { label: "통계 · 성과", icon: BarChart3, href: "/institution/workspace/stats" },
    { label: "기관 설정", icon: Settings, href: "/institution/workspace/settings" },
]
                    ...
                    {SIDEBAR_MENU.map((item) => {
                        ...
                        return (
                            <Link
                                key={item.label}
                                href={item.href}
                                className={`flex items-center gap-3 px-3 py-3 rounded-lg text-base font-semibold transition-all group ${isActive
                                    ? "bg-slate-100 text-slate-900 shadow-sm"
                                    : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"
                                    } ${!isSidebarOpen && "justify-center"}`}
                                title={!isSidebarOpen ? item.label : ""}
                            >
                                <item.icon size={22} className={`flex-shrink-0 ${isActive ? "text-[#4F5B75]" : "group-hover:text-slate-700"}`} />
                                {isSidebarOpen && (
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        {isSidebarOpen && (
                            <div className="text-sm truncate overflow-hidden animate-in fade-in duration-300">
                                ...
                                <p className="text-slate-400 text-xs font-bold">시스템 정상 가동 중</p>
                            </div>
                        )}
                        {isSidebarOpen && (
                            <Link href="/logout" className="ml-auto text-slate-400 hover:text-red-500 transition-colors">
                                <LogOut size={14} />
                            </Link>
                        )}
                    </div>
